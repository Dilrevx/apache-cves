...
ROOT = Path("/data/lhq/workspace/ljl-patch-java20")
INPUT = ROOT / "input"
DATASET = INPUT
...
def run_case(cve):
    ...
    if (case_dir / "pom.xml").exists():
        ...
        local_repo = Path(os.environ.get("CVE_POC_MAVEN_REPO", "/data/lhq/workspace/ljl-patch-java20/.m2-poc"))
...
def make_patch(work_dir, patch_path):
    ...
    proc = run(["git", "diff", "--binary", "--", ".cve_poc_local"], cwd=work_dir, check=False)
...
def main():
    ...
    for cve, library, repo, group, artifact in CANDIDATES:
