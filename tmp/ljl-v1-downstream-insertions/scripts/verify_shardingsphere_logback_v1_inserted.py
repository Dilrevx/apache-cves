...
SAMPLE_ID = "CVE-2021-42550__apache_shardingsphere_5_0_0_v1_logback_real_entrypoint"
ROOT = Path("/data/lhq/workspace/ljl-v1-downstream-insertions")
LEGACY_ROOT = Path("/data/lhq/workspace/ljl-strict-redo")
ARCHIVE = LEGACY_ROOT / "historical-scan" / "apache-shardingsphere-5.0.0-shardingsphere-proxy-bin.tar.gz"
...
DIST_NAME = "apache-shardingsphere-5.0.0-shardingsphere-proxy-bin"
TARGET_CORE_NAME = "logback-core-1.2.0.jar"
...
def ensure_distribution(sample_dir):
...
def write_minimal_server_yaml(dist):
    server_yaml = dist / "conf" / "server.yaml"
...
def insert_v1_logback(dist, evidence_dir):
    ...
    server_yaml = write_minimal_server_yaml(dist)
    evidence = {
        "artifact": str(ARCHIVE),
        "source_case": SOURCE_CASE,
        "source_v1": str(SOURCE_V1),
        "insertion_method": (
            "isolated runtime dependency insertion: official Apache ShardingSphere Proxy 5.0.0 "
            "binary with generated-v1-selected vulnerable logback-core 1.2.7 bytes copied into "
            "the original ShardingSphere logback-core 1.2.0 runtime filename before invoking "
            "the real bin/start.sh entrypoint"
        ),
        "before_insertion": {TARGET_CORE_NAME: before},
        "inserted_v1_jars": {TARGET_CORE_NAME: after},
        "runtime_logback_jars_after_insertion": jars,
        "logback_classic_retained": "lib/logback-classic-1.2.0.jar",
        "minimal_server_yaml": str(server_yaml),
        "minimal_server_yaml_contents": server_yaml.read_text(encoding="utf-8"),
    }
...
def exercise_shardingsphere(dist, evidence_dir, case_name, location_factory):
    ...
    (evidence_dir / f"{case_name}-logback.xml").write_text(config.read_text(encoding="utf-8"), encoding="utf-8")
    env = os.environ.copy()
    ...
    thread.join(18)
    stop_leftovers(dist)
    stdout_log = dist / "logs" / "stdout.log"
    log_text = stdout_log.read_text(encoding="utf-8", errors="replace") if stdout_log.exists() else ""
    result = {
        "case": case_name,
        "entrypoint": "bin/start.sh 33319",
        "cmd": ["bash", "bin/start.sh", "33319"],
        ...
        "stdout_contains_bootstrap": "org.apache.shardingsphere.proxy.Bootstrap.main" in log_text,
        "stdout_contains_jndi_connection_source": "JNDIConnectionSource" in log_text,
        "stdout_tail": log_text[-10000:],
    }
...
def main():
    ...
    dist = ensure_distribution(sample_dir)
    dependency_evidence = insert_v1_logback(dist, evidence_dir)
    positive = exercise_shardingsphere(
        dist,
        evidence_dir,
        "positive",
        lambda port: f"rmi://127.0.0.1:{port}/item",
    )
    negative = exercise_shardingsphere(
        dist,
        evidence_dir,
        "negative",
    ...
    confirmed = (
        ...
        and positive["stdout_contains_bootstrap"]
        and negative["stdout_contains_bootstrap"]
    ...
    result = {
        ...
        "downstream_repo": "apache/shardingsphere",
        "downstream_application": "Apache ShardingSphere Proxy 5.0.0",
        "downstream_entrypoint": "bin/start.sh 33319",
        "downstream": "Apache ShardingSphere Proxy 5.0.0",
        "attack_surface": (
            "ShardingSphere Proxy's real startup script loads conf/logback.xml during Bootstrap.main; "
            "the inserted v1 logback-core resolves JNDIConnectionSource before the proxy is terminated"
        ),
        "cve": "CVE-2021-42550",
        ...
        "verifier": str(root / "scripts" / "verify_shardingsphere_logback_v1_inserted.py"),
        ...
        "notes": (
            ...
            "an isolated Apache ShardingSphere Proxy distribution, then triggers JNDIConnectionSource "
            "through ShardingSphere's real bin/start.sh launcher and logback configuration path. "
    ...
    (evidence_dir / "run.rc").write_text(
        ...
        f"positive_bootstrap_observed={int(positive['stdout_contains_bootstrap'])}\n"
        ...
        f"negative_bootstrap_observed={int(negative['stdout_contains_bootstrap'])}\n"
    ...
    (sample_dir / "README.md").write_text(
        f"# {SAMPLE_ID}\n\n"
        "This is a strict insertion sample. The verifier unpacks the official Apache ShardingSphere "
        "Proxy 5.0.0 binary into an isolated runtime, copies the vulnerable logback-core 1.2.7 bytes "
        "selected by `logback/CVE-2021-42550/v1` into ShardingSphere's original "
        "`logback-core-1.2.0.jar` classpath filename, writes a minimal valid `conf/server.yaml`, "
        "and starts the real `bin/start.sh` launcher.\n\n"
        ...
        "Run: `python3 scripts/verify_shardingsphere_logback_v1_inserted.py --fresh` from "
